...
format: 
   odt:
      ...
      fig-dpi: 300
      fig-width: 6.3
      reference-doc: style-odt.odt
crossref:
  fig-prefix: figure   # (défaut: "Figure")
  tbl-prefix: table    # (défaut: "Table")
  ref-hyperlink: false
#editor: visual
lang: fr
...
Ces différents éléments anatomiques, ainsi que l'environnement dans lequel prospère le champignon, représentent autant de caractéristiques permettant l'identification d'une espèce, et qui seront exploitées dans cette étude (voir [figure @fig-AnatomieChampis}]).^[Voir également annexes, page \pageref{chapitre:annexemyco} et suivantes.]

![Eléments anatomiques du sporophore et caractéristiques exploitées pour son identification.](AnatomieChampis.jpg){#fig-AnatomieChampis}
...
```{r Lots-Chrono, echo = FALSE, fig.height = 4, out.width = "100%", fig.cap = paste0("Temps de calcul des principales distributions d'intérêt pour ", n_chrono, " facteurs, (", fois_chrono, " iter.)")}
...
```{r Lots-Dispersion2D, echo = FALSE, fig.height = 3.2,  fig.cap = paste0("Nuages de points de 2 paramètres de taille (Ls et Dc), sans dispersion (à gauche) et avec dispersion (à droite), pour ", n_reduit, " champignons")}
plot(ggarrange(
   ncol = 2,
...
```{r Lots-DispersionDensite, echo = FALSE, fig.height = 3.2, fig.width = 5.5, fig.cap = 'Diagramme de densité de 2 paramètres de taille, avec dispersion'}
plot(densite2d)
```
...
```{r HorsNormes, echo = FALSE, fig.height = 3.2, fig.width = 5.5, fig.cap = paste0("Distribution du diamètre de stipe Ds, pour Dsmax = ", Chap.Diam)}
...
```{r, BIrpartcostSenSpe, echo = FALSE, fig.height = 4, fig.cap = "Sensibilité (à gauche) et spécificité (à droite) de rpartCost en classification binaire, en fonction de la complexité et du coût (interpolation Kriging quadratique, points expérimentaux encadrés en noir)"}
plot(ggarrange(
   ncol = 2,
...
```{r, echo = FALSE, fig.height = 4, fig.width = 5.2, fig.cap = paste0("Performances (indice de Youden pondéré ", Jw_ratio,":1) de rpartCost en classification binaire, en fonction des hyperparamètres réduits de complexité X1 et de coût X2 (interpolation Kriging quadratique, points expérimentaux encadrés en noir)")}
...
```{r, echo = FALSE, fig.height = 4, fig.cap = "Sensibilité (à g.) et spécificité (à d.) du modèle Rborist en classification binaire, en fonction de ses hyperparamètres (interpolation Kriging quadratique, points expérimentaux en noir)"}
plot(ggarrange(
   ncol = 2,
...
```{r, echo = FALSE, fig.height = 4, out.width = "100%", fig.width = 5.2, fig.cap = "Performance (indice de Youden pondéré) du modèle Rborist en classification binaire, en fonction de ses hyperparamètres réduits : nombre de prédicteurs X1 et de nombre de références avant scission X2 (interpolation Kriging quadratique, points expérimentaux encadrés en noir)"}
...
```{r, MULFAM-rpart, echo = FALSE, fig.height = 3, fig.width = 5, fig.cap = "Performance de rpart en classification par famille, selon le paramètre de complexité"}
...
```{r, echo = FALSE, fig.height = 4, fig.cap = "Performances du modèle Ranger dans une classification par familles, en fonction de ses hyperparamètres (algorithme de scission\ : Gini à gauche, extratrees à droite)"}
plot(ggarrange(
   ncol = 2,
...
```{r, MULFAM2DRborist, echo = FALSE, fig.height = 4, fig.width = 5.2, fig.cap = "Performances du modèle Rborist dans une classification par familles, en fonction de ses deux hyperparamètres (interpolation Kriging quadratique, points expérimentaux encadrés en noir)"}
...
```{r, echo = FALSE, fig.height = 4, fig.cap = "Performances du modèle Ranger dans une classification par espèces, en fonction de ses hyperparamètres (algorithme de scission\ : Gini à gauche, extratrees à droite)"}
plot(ggarrange(
   ncol = 2,
...
```{r, echo = FALSE, fig.height = 4*.9, fig.width = 5.2*.9, fig.cap = "Performances du modèle Rborist dans une classification par espèces, en fonction de ses deux hyperparamètres (interpolation Kriging quadratique, points expérimentaux encadrés en noir)"}
...
```{r Annexe-Des, echo = FALSE, fig.height = 4, fig.cap = "Exemples de lois de densité obtenues avec des jets de dés"}
plot(OUT_graphe_des)
```
...
```{r, echo = FALSE, fig.height = 2.5, fig.cap = "Exemples de lois uniforme (à gauche) et normales (à droite)"}
plot(ggarrange(
   ncol = 2, widths = c(1,1),
   OUT_graphe_uniforme,
   OUT_graphe_normale))
...
```{r, echo = FALSE, fig.height = 2.5, fig.cap = "Exemples de lois binomiale (à gauche) et de Weibull (à droite)"}
plot(ggarrange(
   ncol = 2, widths = c(1,1),
...
```{r, echo = FALSE, fig.height = 4, fig.cap = "Exemples de lois bêta"}
...
```{r Annexe-LDAMono, echo = FALSE, fig.height = 2.8, fig.cap = "Distributions unimodales et critère de séparation (en pointillés)"}
plot(ggarrange(
   ncol = 2, widths = c(1,1),
...
```{r Annexe-LDABi, echo = FALSE, fig.height = 2.8, fig.cap = "Distributions bimodales et critère de séparation (en pointillés)"}
plot(ggarrange(
   ncol = 2, widths = c(1,1),
   OUT_graphe_bimodale,
...
```{r, echo = FALSE, fig.height = 3, fig.width = 5, fig.cap = "Graphique des performances de rpart"}
plot(BI_fit_rpart_cp_graphe)
```
