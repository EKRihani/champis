...
La deuxième limite, plus profonde, est la logique inhérente au processus évolutionnaire. Deux phénomènes antagonistes sont en jeu\ : convergence et divergence évolutives. Ces deux phénomènes sont liés à la nécessaire adaptation des espèces à leurs environnements. 
...
* Détailler et mettre en œuvre des stratégies et méthodes de création de lots de données synthétiques, dont l'intérêt et les applications s'étendent bien au delà du domaine de la mycologie,
...
Le sporophore "typique" est constitué d'un *stipe* (pied), surmonté d'un *chapeau*, ce dernier portant en sa face inférieure un *hyménophore*, souvent constitué de *lames* ou de *tubes*, portant les spores qui, une fois disséminées, donneront naissance à de nouveaux individus.
...
Nous pouvons alors modéliser $L_{S}$, $D_{S}$ et $D_{C}$ par les lois suivantes\ :
...
```{r Lots-Chrono, echo = FALSE, fig.height = 4, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = paste0("Temps de calcul des principales distributions d'intérêt pour ", n_chrono, " facteurs, (", fois_chrono, " iter.)")}
...
Ainsi, la génération de la variable aléatoire $F_{T}$ obéira idéalement à une loi de distribution asymétrique vers la droite. Ce critère d'asymétrie écarte par conséquent les lois de distribution symétriques telles que la loi normale ou la loi uniforme.

```{r Lots-DistribSym, echo = FALSE, fig.height = 3.5, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Exemples de distributions de la loi uniforme (à gauche), binomiale (au centre) et normale (à droite)"}
plot(ggarrange(
   widths = c(1.1, 1, 1),
   ncol = 3,
...
```{r Lots-DistribAsym, echo = FALSE, fig.height = 3.5, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Exemples de distributions de la loi de Poisson (à gauche), de Weibull (au centre) et bêta non-centrale (à droite)"}
plot(ggarrange(
   widths = c(1.1, 1, 1),
   ncol = 3,
...
```{r Lots-Dispersion2D, echo = FALSE, fig.height = 3.2,  out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = paste0("Nuages de points de 2 paramètres de taille (Ls et Dc), sans dispersion (à gauche) et avec dispersion (à droite), pour ", n_reduit, " champignons")}
plot(ggarrange(
   ncol = 2,
...
```{r Lots-DispersionDensite, echo = FALSE, fig.height = 3.2, fig.width = 5.5, dev = "jpeg", dpi = 600, fig.cap = 'Diagramme de densité de 2 paramètres de taille, avec dispersion'}
plot(densite2d)
```

Une simulation de Monte Carlo unidimensionnelle effectuée sur `r INTRO_n_champis` spécimens nous permet ainsi d’évaluer la proportion de spécimens "hors normes" dépassant la valeur dimensionnelle maximale à environ `r INTRO_taux_gros_diam` % (voir figure \ref{fig:HorsNormes}).^[En pratique, la méthode de Monte Carlo consiste ici à générer un grand nombre de champignons suivant les lois mathématiques que nous avons définies précédemment, puis à dénombrer ceux répondant aux critères d'intérêt, c'est-à-dire ceux ayant des dimensions dépassant les valeurs maximales usuelles.] La même simulation nous permet d'évaluer la proportion de spécimens "exceptionnels", dépassant de plus de 10% cette valeur maximale, à moins de `r INTRO_taux_supergros_diam` %.

```{r HorsNormes, echo = FALSE, fig.height = 3.2, fig.width = 5.5, dev = "jpeg", dpi = 600, fig.cap = paste0("Distribution du diamètre de stipe Ds, pour Dsmax = ", Chap.Diam)}
...
Le lot de données ainsi obtenu contient $N = `r BI_n_champis`$ spécimens, de $n = `r GEN_n_especes`$ espèces, caractérisés par 25 facteurs morphologiques ou environnementaux, qualitatifs ou quantitatifs.
...
1. Entraînement\ : le modèle d'apprentissage machine est exposé à un *jeu de données d'entraînement* (*training data set*), censé être représentatif des données auquel le modèle sera exposé en utilisation réelle. Cette phase est la phase d'apprentissage du modèle.^[Voir section \ref{chapitre:lotsynth}, page \pageref{chapitre:lotsynth}]
...
![Principe de scissions successives d'un jeu de données initial en jeux d'apprentissage, d'optimisation et d'évaluation.](Split-Jeux.svg)

 \label{paragraphe:split_ratio}
...
![Principe de la validation croisée à k blocs (*k-fold cross-validation*), pour *k* = 4.](Cross-Validation.svg)
...
L'analyse discriminante linéaire est une méthode ayant été proposée par Ronald Fisher en 1936[@fisher_use_1936;@anderson_r_1996] pour résoudre des problèmes de classification taxonomique dans le domaine de la botanique.^[Cette étude, proposant une méthode de classification des 3 variétés *Iris setosa*, *Iris virginica* et *Iris versicolor* en fonction de 4 paramètres dimensionnels (longueur et largeur, des sépales et pétales) est par ailleurs à l'origine du célèbre jeu de données *Iris*.] La LDA est basée sur la construction de l'hyperplan de projection permettant de maximiser la distance entre les moyennes projetées des différentes classes et de minimiser la variance intraclasse (voir figure \ref{fig:Principe-LDA}).[@hastie_elements_2016] La LDA peut être utilisée à fins de classification, mais aussi pour effectuer des réductions de dimensionnalité ou encore afin de faciliter l'interprétation de l'importance de certaines caractéristiques.

![Séparation par distance maximale des moyennes interclasses (à gauche), et par projection sur l'hyperplan optimal tenant compte des variances intraclasses (LDA, à droite).](LDA.svg)
...
$$D = \sum_{i=1}^n \lambda{}_{i}.d_{i} \quad\text{avec}\quad d_{i} = \overline{x_{i, a}} - \overline{x_{i, b}}$$

$\overline{x_{i, a}}$ et $\overline{x_{i, b}}$ étant les moyennes respectives de chaque caractéristique $x_{i}$ pour les groupes (espèces) $a$ et $b$, et :
...
pander::pander(iris_M, caption = "Moyennes et différences de moyennes des 4 paramètres d'Iris setosa et versicolor")
```

```{r, LDA-TableProd, echo = FALSE}
pander::pander(iris_prods, caption = "Produits des différences à la moyenne des 4 paramètres d'Iris setosa et versicolor ($S_{pq}$)")
```
...
```{r LDA-MinMax, echo = FALSE, fig.height = 2.3, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Distribution des variétés setosa et versicolor en fonction de leurs caractéristiques (paramètres fortement pondérés à gauche, faiblement pondérés à droite)"}
...
```{r LDA-Separation, echo = FALSE, fig.height = 2.3, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Distribution de X (à gauche) et des paramètres dimensionnels normalisés (à droite)"}
plot(ggarrange(widths = c(1, 1.5),
   ncol = 2,
...
* Multivariés, qui exploitent plus d'une caractéristique. Le cas le plus habituel est la scission oblique (*oblique split*), basée sur un hyperplan représentant une combinaison linéaire de caractéristiques. Ce type de test multivarié se rapproche ainsi de la LDA.^[cf. section \ref{paragraphe:Modeles_LDA}, page \pageref{paragraphe:Modeles_LDA}]

Les différents modèles d'arbres de classification pourront mettre en œuvre des tests purement univariés (ID3, C4.5, CHAID...), purement obliques (arbres dits *perceptron*) ou autoriser les deux types de tests (arbres CART\ : *Classification And Regression Trees*).[@loh_fifty_2014]
...
Pour l'arbre CART qui sera utilisé dans la classification binaire de cette étude, le critère d'impureté mise en œuvre au sein du modèle est le coefficient de Gini\ :[@wu_top_2009]
$$G = 1- p^{2} - \left(1- p \right)^{2}$$
avec $p$ la fréquence relative de l'une des deux classes, au sein du nœud considéré.

Les arbres obtenus peuvent enfin être rationalisés à l'aide d'un algorithme d'élagage (*pruning*) visant à réduire la complexité de l'arbre. Cet algorithme effectue l'analyse de l'arbre des feuilles vers la racine -- donc dans le sens inverse de l'induction qui l'a construit -- et en évaluant si chaque nœud intermédiaire pourrait être avantageusement remplacé par une feuille ou par une branche. Différents critères d'optimisation existent, parmi lesquels nous pouvons citer l'élagage coût-complexité,[@kretowski_evolutionary_2019] exploité par le modèle CART, et qui vise à minimiser le facteur coût-complexité $CC(T)$ de notre arbre $T$\ :
$$CC(T) = Erreur(T) + \alpha{}\times Taille(T)$$
avec $\alpha{}$ un hyperparamètre de complexité^[cf. annexes, section \ref{paragraphe:train-CART}, page \pageref{paragraphe:train-CART} et suivantes] strictement positif évalué expérimentalement.^[cf. section \ref{chapitre:doe}, page \pageref{chapitre:doe}]

Les résultats de la mise en œuvre d'une classification par arbre de décision sur le jeu de données Iris sont représentés par la figure \ref{fig:Iris-CART}. 

Des exemples d'application sur les macromycètes sont également illustrés par les figures \ref{fig:Iris-Champis-CARTa} et \ref{fig:Iris-Champis-CARTb}, qui permettent d'appréhender la potentielle complexité de la classification des champignons, même en limitant la prédiction à un critère binaire (comestible ou toxique).^[Les nœuds aboutissant à des feuilles de type *100\% comestibles* ont été censurés.]

![Arbre et critères de classification des trois espèces du lot de données Iris.](IrisCARTDual.svg)

![Arbre simplifié proposé par CART pour la classification des champignons.](IntroChampisCART2Arbre.svg)

![Structure arborescente complète proposée par le modèle CART pour la classification des champignons.](IntroChampisCART1Arbre.svg)
...
* Apparition, grâce notamment aux avancées de la génomique, de problèmes dits de type *grand p, petit n*, avec un grand nombre de prédicteurs $p$ pour un petit nombre d’observations $n$, qui mettent en échec la vision classique de parcimonie d’un modèle unique pour un problème unique,^[En l'espèce, les problèmes posés par la génomique imposent souvent d'analyser des dizaines de milliers de gènes (*p*), sur quelques dizaines ou centaines de patients (*n*).]
...
Une forêt aléatoire est un type particulier de forêt, dont la définition canonique correspond, d’après Breiman, à une collection $\left(\widehat{h}(., \Theta_{1}),\dots, \widehat{h}(., \Theta_{q}) \right)$ d’arbres de prédiction avec $\Theta_{1},\dots, \Theta_{q}$ des variables indépendantes, identiquement distribuées, aléatoirement sélectionnées dans le lot de données $\mathcal{L}_{n}$ comptant $n$ observations et $q$ prédicteurs.[@breiman_random_2001; @genuer_random_2020]
...
Le type le plus commun de forêt aléatoire est la forêt aléatoire à entrées aléatoires (RF-RI\ : *Random Forests Random Inputs*).^[Les forêts RF-RI sont d’ailleurs le type de forêt aléatoire auquel il est souvent tacitement référence lorsque le terme *forêt aléatoire* est utilisé dans la littérature.] Le terme *entrées aléatoires* est ici à interpréter comme *variables d’entrées aléatoires*\ : le principe de la forêt RF-RI est de construire une forêt aléatoire avec des prédicteurs arborescents dont les variables d’entrées sont aléatoires, chacun construit à partir d’un échantillon bootstrapé.^[L’échantillonnage bootstrap correspond à la construction d’un échantillon par prélèvement d’individus *avec* remplacement.]
...
3. Poursuivre le partitionnement récursif jusqu’à la création d’un arbre^[cf. section \ref{paragraphe:Modeles_arbres}, page \pageref{paragraphe:Modeles_arbres}]\ : $\widehat{h}(., \Theta_{\ell}), 1 \leq \ell \leq q$,
...
![Algorithme de génération des forêts aléatoires de type RF-RI.](RandomForest.svg)
...
```{r Carres-Latins, echo = FALSE, fig.height = 2.5, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Carré latin aléatoire (à gauche), carré latin avec optimisation évolutive ESE maximin (au milieu), carré latin quasi-orthogonal (à droite)"}
plot(ggarrange(
   widths = c(1.05,1,1),
   ncol = 3,
   graphe_LHS + ylab("X2"),
...
D'autres indicateurs d'intérêt existent, nous retiendrons ici l'indice de Youden pondéré ($J_{w}$), qui permet d'ajuster l'importance relative accordée à la spécificité et à la sensibilité, au sein d'un index synthétique.[@youden_index_1950; @rucker_summary_2010] Cet indicateur présente un intérêt particulier lorsqu'il apparaît souhaitable de tenir compte de la différence d'impact entre un faux positif et un faux négatif, sans pour autant autoriser des sensibilités ou spécificités trop faibles.
...
Nous retiendrons dans les classifications multiclasses effectuées au cours de notre étude le kappa de Cohen, calculé à partir de la matrice de confusion (illustrée en figure \ref{fig:Matrice-Confusion}), et donné par\ :[@cohen_coefficient_1960]
...
![Extrait d'une matrice de confusion, pour une classification multiclasse.](Matrice-Confusion.svg)
...
L'interprétation du kappa pouvant parfois être assez contre-intuitive, cette étude la complétera parfois par l'indice de Rand $R$, métrique moins robuste en présence de données non-équilibrées,^[Ce cas se présente habituellement lors d'une surreprésentation de certaines classes dans les jeux de données.] mais présentant l'avantage d'être de compréhension plus aisée, car représentant le pourcentage de prédictions exactes.
\newpage
...
Les modèles sélectionnés pour cette étude sont de types variés\ :^[Les termes entre parenthèses font référence aux noms de modèles qu'utilise la librairie *caret*.]
...
1. Découpage du lot de données en un jeu d'entraînement/optimisation et en un jeu de validation, avec adaptation des rapports de taille en fonction du volume de données du lot initial,^[cf. section \ref{chapitre:split}, page \pageref{paragraphe:split_ratio}]
2. Apprentissage sur le jeu d'entraînement, exploitant une validation croisée à k blocs, avec adaptation du nombre de blocs à la taille du lot de données,^[cf. section \ref{chapitre:split_methodes}, page \pageref{paragraphe:crossvalid}]
3. Exploration de l'ensemble de l'espace expérimental des hyperparamètres du modèle, via la méthode des hypercubes latins quasi-orthogonaux,^[\label{note:doe}cf. section \ref{chapitre:doe}, page \pageref{chapitre:doe}]
4. Mesure des performances en exploitant une métrique adaptée,^[cf. section \ref{chapitre:perf}, page \pageref{chapitre:perf}]
5. Modélisation des performances en fonction des hyperparamètres, via un modèle quadratique avec interactions,
...
pander::pander(NAIF_Resultats, caption = "Performances de différents classifieurs naïfs")
```  
...
Les performances du modèle monovariable ne sont guère plus élevées, ce qui démontre l'inefficacité des adages populaires prétendant garantir la comestibilité de tous les champignons d'une couleur donnée.^[Même si le modèle venait à montrer le contraire, il convient de rappeler que ce lot de données se limite aux espèces les plus courantes, d'une fonge limitée dans l'espace et dans le temps.]

En revanche, les performances du modèle bivariable sont honorables, avec une amélioration sensible de la spécificité ($Spec \approx `r NAIF_Resultats["BiCritere","Spec"]`$), donc de la capacité à réellement distinguer des champignons comestibles. Les performances de ce modèle ($J_w = `r NAIF_Resultats["BiCritere","Jw"]`$) ne répondent toutefois pas au critère de performance défini précédemment ($J_{w} \geq `r Jw_min`$).^[cf. section \ref{chapitre:perf}, page \pageref{chapitre:perf}] Les performances calculatoires sont extrêmement médiocres ($t > `r floor(NAIF_Resultats["BiCritere","Temps (s)"]/3600)`$ h), et probablement à mettre en relation avec un code peu optimisé.
...
```{r, BIldapda, echo = FALSE, fig.height = 3, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Performances des modèles lda2 (à gauche) et pda (à droite), dans une tâche de classification binaire"}
plot(ggarrange(widths = c(1, 1.5),
   ncol = 2,
...
```{r, BIldacoef, echo = FALSE, fig.height = 2.7, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Dix premiers facteurs de LDA prédisant la non-comestibilité des champignons"}
...
```{r, BIrpart, echo = FALSE, fig.height = 3, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Performances du modèle rpart en classification binaire, en fonction du paramètre de complexité (cp)"}
...
```{r, BIrpartcostSenSpe, echo = FALSE, fig.height = 4, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Sensibilité (à gauche) et spécificité (à droite) de rpartCost en classification binaire, en fonction de la complexité et du coût (interpolation Kriging quadratique, points expérimentaux encadrés en noir)"}
plot(ggarrange(
   ncol = 2,
...
```{r, echo = FALSE, fig.height = 4, out.width = "100%", dev = "jpeg", dpi = 600, fig.width = 5.2, fig.cap = paste0("Performances (indice de Youden pondéré ", Jw_ratio,":1) de rpartCost en classification binaire, en fonction des hyperparamètres réduits de complexité X1 et de coût X2 (interpolation Kriging quadratique, points expérimentaux encadrés en noir)")}
plot(BI_fit_rpartcost_jw_graphe + theme(legend.position='right') + theme(legend.text = element_text(angle = 0, vjust = 1, hjust = 0)))
```

![Arbres simplifiés, optimisant la sensibilité (à g.) ou la spécificité (à d.)](BI_Arbre_Rpartcost_Dual.svg)


Les arbres de classification les plus simples obtenus (figure \ref{fig:ArbresRpartCost}) avec des valeurs d'hyperparamètre *Cost* se situant aux limites de notre espace expérimental présentent ainsi des structures et des critères assez différents, selon que l'on optimise la sensibilité pour obtenir un arbre "conservateur", ou la spécificité pour obtenir un arbre "optimiste" (voire suicidaire).^[Les nœuds aboutissant à des feuilles de type *100\% comestibles* ont été censurés.]

Nous pouvons notamment remarquer la forte prévalence de nœuds intermédiaires dotés de valeurs *Rejeter* ou *Conserver*, selon que l'accent ait été porté sur la sensibilité (tendance au rejet) ou la spécificité (tendance à la consommation).

Contrairement à la LDA, il n'est pas possible de déterminer les critères les plus discriminants. Les critères apparaissant dans les étages supérieurs de l'arbre sont ceux permettant de mieux séparer l'ensemble des individus en sous-ensembles les plus homogènes possibles, sans pour autant s'avérer décisifs dans la classification, dans un sens comme dans l'autre.
...
Nous pouvons modéliser la réponse Y ($J_{w}$) par un modèle quadratique avec interaction\ :^[cf. section \ref{chapitre:doe}, page \pageref{chapitre:doe}]
...
Bien que ne disposant d'aucun hyperparamètre, ce modèle a donné d'excellents résultats, frôlant le critère $J_w \geq `r Jw_min`$, sans aucune optimisation nécessaire. En outre, ce modèle s'illustre par une sensibilité -- c'est à dire une capacité à rejeter les champignons non comestibles -- particulièrement élevée.

Le temps de calcul est plus long ($t = `r BI_temps_c50tree`$ min), signe d'une efficience calculatoire moindre, mais cette remarque est à modérer en raison des gains que peut apporter un modèle permettant à l'utilisateur de s'affranchir de la phase d'optimisation des hyperparamètres\ :
...
```{r, echo = FALSE, fig.height = 3, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Performances du modèle de fougères aléatoires, en classification binaire"}
...
```{r, echo = FALSE, fig.height = 4, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Sensibilité (à g.) et spécificité (à d.) du modèle Rborist en classification binaire, en fonction de ses hyperparamètres (interpolation Kriging quadratique, points expérimentaux en noir)"}
plot(ggarrange(
   ncol = 2,
...
Nous pouvons modéliser la réponse par un modèle quadratique avec interaction^[cf. section \ref{chapitre:doe}, page \pageref{chapitre:doe}]\ : \label{paragraphe:ModelBIRborist}
...
```{r, BI-ranger-gini, echo = FALSE, fig.height = 4, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Sensibilité (à gauche) et spécificité (à droite) du modèle Ranger en classification binaire, en fonction des 2 hyperparamètres (algorithme de scission\ : Gini)"}
plot(ggarrange(
   ncol = 2,
...
```{r, BI-ranger-ET, echo = FALSE, fig.height = 4, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Sensibilité (à gauche) et spécificité (à droite) du modèle Ranger en classification binaire, en fonction des 2 hyperparamètres (algorithme de scission\ : extratrees)"}
plot(ggarrange(
   ncol = 2,
...
Avec Y l'indice de Youden pondéré, $X_{1}$ le facteur réduit associé au paramètre de nombre de caractéristiques à séparer à chaque nœud (*mtry*), $X_{2}$ le facteur réduit associé au paramètre de taille minimale de nœud (*min.node.size*), $X_{3}$ le facteur régissant la règle de séparation (*splitrule*, la valeur 0 étant attribuée à *gini*, 1 à *extratrees*) et $b_{n}$ les estimations des coefficients des effets. Le facteur $X_{3}$ n'ayant que deux niveaux, il est évidemment impossible de lui attribuer une composante quadratique.
...
Ces résultats soulignent un fait intéressant\ : tous les modèles de forêts aléatoires ne sont pas égaux. Notre étude montre une différence considérable en sensibilité et spécificité entre les forêts aléatoires de type rFerns, Ranger et Rborist. 
...
Tous les modèles ont été entraînés sur le jeu de données d'apprentissage, après application des hyperparamètres optimaux obtenus précédemment par modélisation des performances via un modèle quadratique avec interactions.^[cf. section \ref{chapitre:BI-RF}, page \pageref{chapitre:BI-RF}] Les performances de nos modèles face au jeu de données d'évaluation, auquel ils n'ont encore jamais été exposés, seront évaluées avec le même critère que précédemment\ : $J_{w} \geq `r Jw_min`$.^[cf. section \ref{chapitre:split_methodes}, page \pageref{chapitre:split_methodes}]
...
```{r BI-CM, echo = FALSE}
pander::pander(BI_CM_ranger_final$table, caption = "Matrice de confusion du modèle Ranger (prédictions à gauche, référence en haut)")
```  
...
pander::pander(BI_RF_resultat, digits = 4, caption = "Performances des modèles Ranger et Rborist (jeu d'évaluation)")
```
...
Étant données les performances des différents modèles en classification binaire, seuls les modèles basés sur les arbres décisionnels et les forêts aléatoires seront évalués dans cette section.

## Classification par familles

### Notion de famille

Dans le cadre de cette étude, nous n'utilisons pas la notion de famille au sens phylogénétique du terme. Nous n'utiliserons pas non plus d'autres taxons afin de regrouper les espèces.

Ce choix est dicté par les nécessités suivantes \:

* Limiter l'hétérogénéité entre classes,
* Éviter un trop grand nombre de classes, dont certaines ne contiendraient qu'une espèce,
* Inversement, éviter d'avoir un nombre trop faible de classes, dont certaines contiendraient un nombre considérable d'espèces.

Nous avons retenu, pour cette étude, une classification plus souple, qui est celle proposée dans le Photoguide du Pr Courtecuisse.[@courtecuisse_photo-guide_2000] Les familles représentées ici peuvent être retrouvées dans la matrice de confusion située à la fin de cette section (table \ref{tab:FAM-CM}, page \pageref{tab:FAM-CM}).

### Modèles d'arbres de décision

Le modèle d'arbre de décision présenté dans cette partie est rpart, le plus simple des modèles CART. Ce modèle dispose d'un unique hyperparamètre \ : *cp* (complexité). Les performances maximales sont obtenues pour de faibles valeurs de l'hyperparamètre *cp* (figure \ref{fig:MULFAM-rpart}).

```{r, MULFAM-rpart, echo = FALSE, fig.height = 3, fig.width = 5, dev = "jpeg", dpi = 600, fig.cap = "Performance de rpart en classification par famille, selon le paramètre de complexité"}
plot(MULFAM_fit_rpart_cp_graphe)
```

Ce modèle, pourtant très simple, donne déjà de très bons résultats globaux, avec un kappa ($\kappa_{max} =$ `r round(max(MULFAM_fit_rpart_cp_resultats["Kappa"]),3)`) et une précision ($R_{max} =$ `r round(max(MULFAM_fit_rpart_cp_resultats["Accuracy"]),3)`) élevés.
...
```{r, echo = FALSE, fig.height = 4, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Performances du modèle Ranger dans une classification par familles, en fonction de ses hyperparamètres (algorithme de scission\ : Gini à gauche, extratrees à droite)"}
plot(ggarrange(
   ncol = 2,
...
Comme précédemment, nous pouvons proposer pour ce modèle la modélisation quadratique suivante\ :^[cf. section \ref{paragraphe:ModelBIRanger}, page \pageref{paragraphe:ModelBIRanger}] \label{paragraphe:ModelMULFAMRanger}
$$ Y = b_{0} + b_{1}.X_{1} + b_{2}.X_{2} + b_{3}.X_{3} + b_{12}.X_{1}.X_{2} + b_{23}.X_{2}.X_{3} + b_{13}.X_{1}.X_{3} + b_{11}.X_{1}^{2} + b_{22}.X_{2}^{2}$$
Avec Y le kappa, $X_{1}$ le facteur réduit associé au paramètre de nombre de caractéristiques à séparer à chaque nœud (*mtry*), $X_{2}$ le facteur réduit associé au paramètre de taille minimale de nœud (*min.node.size*), $X_{3}$ le facteur régissant la règle de séparation (*splitrule*, la valeur 0 étant attribuée à *gini*, 1 à *extratrees*) et $b_{n}$ les estimations des coefficients des effets.
...
Après optimisation des hyperparamètres, ce modèle a donné d'excellents résultats dans la classification par familles de notre jeu de données d'essai.

```{r, echo = FALSE}
pander::pander(MULFAM_fit_ranger_best_resultats[c(1,2,3,7,8)], digits = 5, caption = "Performances du modèle Ranger (hyperparamètres optimaux)")
```
...
```{r, MULFAM2DRborist, echo = FALSE, fig.height = 4, fig.width = 5.2, dev = "jpeg", dpi = 600, fig.cap = "Performances du modèle Rborist dans une classification par familles, en fonction de ses deux hyperparamètres (interpolation Kriging quadratique, points expérimentaux encadrés en noir)"}
plot(MULFAM_fit_Rborist_kappa_graphe + theme(legend.position='right')+ theme(legend.text = element_text(angle = 0, vjust = 1, hjust = 0)))
```

Comme lors de la classification binaire,^[cf. section \ref{paragraphe:ModelBIRborist}, page \pageref{paragraphe:ModelBIRborist}] nous pouvons modéliser la réponse par un modèle quadratique avec interaction\ :^[cf. section \ref{chapitre:doe}, page \pageref{chapitre:doe}] \label{paragraphe:ModelMULFAMRborist}
$$ Y = b _{0} + b _{1}.X_{1} + b_{2}.X_{2} + b_{12}.X_{1}.X_{2} + b _{11}.X_{1}^{2} + b_{22}.X_{2}^{2}$$
avec $Y$ le kappa, $X_{1}$ le facteur réduit dans la plage [0;1] associé à l'hyperparamètre de nombre de prédicteurs testés par scission (*predFixed*), $X_{2}$ le facteur réduit associé à l'hyperparamètre de nombre minimal de références distinctes avant scission (*minNode*) et $b_{n}$ les coefficients associés aux effets.

L'interprétation graphique de l'exploration des hyperparamètres (figure \ref{fig:MULFAM2DRborist}, page \pageref{fig:MULFAM2DRborist}) met en évidence, outre la vaste plage d'hyperparamètres permettant d'obtenir une précision maximale, une prévalence de l'hyperparamètre associé au nombre de prédicteurs (*predFixed*) dans l'efficacité de cet algorithme.

Le calcul numérique nous permet de préciser l'effet des hyperparamètres grâce aux estimations de leurs coefficients\ :
...
La performance mesurée sur lot d'entraînement avec hyperparamètres optimaux (*predFixed* = `r MULFAM_best_Rboristgrid$predFixed` et *minNode* = `r MULFAM_best_Rboristgrid$minNode`) est synthétisée dans la table \ref{tab:FAM-PerfRborist}.

```{r FAM-PerfRborist, echo = FALSE}
pander::pander(MULFAM_fit_Rborist_best_resultats[c(1,2,6,7)], digits = 5, caption = "Performances du modèle Rborist (hyperparamètres optimaux)")
```

Rborist a donné des résultats similaires à ceux de Ranger, avec d'excellentes performances.
...
La forêt aléatoire de type Rborist a donné des résultats similaires, avec une précision finale égale à `r MULFAM_CM_Rborist_final$overall["Accuracy"]`, avec un intervalle de confiance à 95% de [`r round(MULFAM_CM_Rborist_final$overall["AccuracyLower"], 4)` ; `r round(MULFAM_CM_Rborist_final$overall["AccuracyUpper"], 4)`]. Le modèle Rborist, donnant des résultats sensiblement identiques à Ranger, s'est avéré nettement moins efficient que ce dernier sur le plan calculatoire (`r MULFAM_temps_Rborist` min).

```{r, echo = FALSE}
pander::pander(MULFAM_RF_resultat, digits = 5, caption = "Performances des modèles Ranger et Rborist (évaluation)")

```

```{r FAM-CM, echo = FALSE}
pander::pander(MULFAM_CM_ranger_final$table, caption = "Matrice de confusion du modèle Ranger (prédictions à gauche, références en haut)")
```
...
```{r, echo = FALSE, fig.height = 4, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Performances du modèle CART (rpart) dans une classification par espèces, en fonction du paramètre de complexité"}
plot(MULESP_fit_rpart_cp_graphe)
```

Face à cette tâche de classification sensiblement plus complexe que la précédente, les performances de ce modèle s'effondrent, avec un kappa ($\kappa_{max} =$ `r round(max(MULESP_fit_rpart_cp_resultats["Kappa"]),3)`) et une précision ($R_{max} =$ `r round(max(MULESP_fit_rpart_cp_resultats["Accuracy"]),3)`) extrêmement faibles.

\newpage
### Forêts aléatoires

Comme lors de la classification multiclasses par familles présentée dans les développements de la section précédente, le premier modèle de forêt aléatoire évalué dans cette partie sera un algorithme de type Ranger. 

L'interprétation des graphiques obtenus suite à l'exploration de l'espace expérimental des hyperparamètres de ce modèle laisse entrevoir de bonnes performances sur une large plage de ce domaine, avec un effet nettement prédominant de *mtry* sur les performances prédictives du modèle.

```{r, echo = FALSE, fig.height = 4, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Performances du modèle Ranger dans une classification par espèces, en fonction de ses hyperparamètres (algorithme de scission\ : Gini à gauche, extratrees à droite)"}
plot(ggarrange(
   ncol = 2,
...
Le calcul numérique des coefficients de la modélisation quadratique du kappa en fonction des hyperparamètres réduits de *mtry* ($X_{1}$, nombre de caractéristiques à séparer à chaque nœud), *min.node.size* ($X_{2}$, taille minimale de nœud) et *splitrule* ($X_{3}$, règle de séparation) nous permet de confirmer l'impact prédominant de l'hyperparamètre *mtry* et de calculer les estimations des effets suivantes\ :^[cf. section \ref{paragraphe:ModelBIRanger}, page \pageref{paragraphe:ModelBIRanger} et section \ref{paragraphe:ModelMULFAMRanger}, page \pageref{paragraphe:ModelMULFAMRanger}]
...
Après optimisation des hyperparamètres (*min.node.size* = `r MULESP_best_rangergrid$min.node.size`, *mtry* = `r MULESP_best_rangergrid$mtry` et *splitrule* = `r MULESP_best_rangergrid$splitrule`), ce modèle a donné, comme lors des tests précédents, d'excellents résultats sur le jeu de données d'entraînement, malgré la complexité accrue du problème.

```{r, echo = FALSE}
pander::pander(MULESP_fit_ranger_best_resultats[c(1,2,3,7,8)], digits = 5, caption = "Performances du modèle Ranger (hyperparamètres optimaux)")
```

Le dernier modèle de forêt aléatoire exploité dans cette tâche de classification par espèces est le Rborist, qui s'était précédemment illustré par d'excellentes performances prédictives.

```{r, echo = FALSE, fig.height = 4*.9, fig.width = 5.2*.9, dev = "jpeg", dpi = 600, fig.cap = "Performances du modèle Rborist dans une classification par espèces, en fonction de ses deux hyperparamètres (interpolation Kriging quadratique, points expérimentaux encadrés en noir)"}
plot(MULESP_fit_Rborist_kappa_graphe + theme(legend.position='right')+ theme(legend.text = element_text(angle = 0, vjust = 1, hjust = 0)))
```

Ce modèle montre, de la même façon que dans nos développements précédents, d'excellentes performances dans un vaste domaine de l'espace des hyperparamètres, avec une prédominance de l'hyperparamètre *predFixed* (nombre de prédicteurs avant scission) sur le kappa.

Comme précédemment, le calcul numérique permet d'estimer les effets des hyperparamètres et de confirmer l'impact de *predFixed* ($X_{1}$)\ :^[cf. section \ref{paragraphe:ModelBIRborist}, page \pageref{paragraphe:ModelBIRborist} et section \ref{paragraphe:ModelMULFAMRborist}, page \pageref{paragraphe:ModelMULFAMRborist}]
...
```{r, echo = FALSE}
pander::pander(MULESP_fit_Rborist_best_resultats[c(1,2,6,7)], digits = 5, caption = "Performances du modèle Rborist (hyperparamètres optimaux)")
```
...
La précision finale du modèle Ranger est égale à $R = `r round(MULESP_CM_ranger_final$overall["Accuracy"], 4)`$, avec un intervalle de confiance à 95% de [`r round(MULFAM_CM_ranger_final$overall["AccuracyLower"], 4)` ; `r round(MULESP_CM_ranger_final$overall["AccuracyUpper"], 4)`]. La forêt aléatoire de type Ranger a donné d'excellents résultats, en un temps très raisonnable (`r MULESP_temps_ranger` min).

La matrice de confusion, ne reprenant ici que les espèces ayant posé problème à notre modèle (`r MULESP_erreur_ranger`:`r MULESP_n_eval`), est résumée par la table \ref{tab:ESP-CM-Ranger} (page \pageref{tab:ESP-CM-Ranger}). Outre le faible nombre d'erreur, nous pouvons remarquer qu'elles ne concernent que des espèces très voisines.

```{r ESP-CM-Ranger, echo = FALSE}
pander::pander(MULESP_CMerreurs_ranger, caption = "Matrice de confusion des erreurs de Ranger, (prédictions à g., références en h.)")
```
...
La précision finale de Rborist est ainsi égale à `r round(MULESP_CM_Rborist_final$overall["Accuracy"],4)`, avec un intervalle de confiance à 95% de [`r round(MULESP_CM_Rborist_final$overall["AccuracyLower"], 4)` ; `r round(MULESP_CM_Rborist_final$overall["AccuracyUpper"], 4)`]. Le modèle Rborist, donnant des résultats marginalement supérieurs à ceux de Ranger, s'est avéré sensiblement moins efficient sur le plan calculatoire (`r MULESP_temps_Rborist` min).

Nous pouvons noter, comme précédemment, que si les performances prédictives des deux modèles sont similaires, le modèle Ranger s'est avéré sensiblement plus rapide que Rborist sur des problèmes complexes.

```{r, echo = FALSE}
pander::pander(MULESP_RF_resultat, digits = 5, caption = "Performances des modèles Ranger et Rborist (évaluation)")
...
Comme évoqué dans nos développements précédents, la génération des données quantitatives est aléatoire, mais obéit à des lois statistiques définies.^[Voir section \ref{chapitre:lotsynth}, page \pageref{chapitre:lotsynth}]
...
```{r Annexe-Des, echo = FALSE, fig.height = 4, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Exemples de lois de densité obtenues avec des jets de dés"}
plot(OUT_graphe_des)
```
...
```{r, echo = FALSE, fig.height = 2.5, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Exemples de lois uniforme (à gauche) et normales (à droite)"}
plot(ggarrange(
   ncol = 2, widths = c(1,1),
   OUT_graphe_uniforme,
   OUT_graphe_normale))
...
* Binomiale, loi discrète ne pouvant prendre que des valeurs entières, et qui peut être approximée par des lois telles que la loi normale ou la loi de Poisson,
...
```{r, echo = FALSE, fig.height = 2.5, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Exemples de lois binomiale (à gauche) et de Weibull (à droite)"}
plot(ggarrange(
   ncol = 2, widths = c(1,1),
...
```{r, echo = FALSE, fig.height = 4, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Exemples de lois bêta"}
plot(OUT_graphe_beta)
```

## Modes et distributions

Lors des développements concernant la LDA (*Linear Discriminant Analysis*), nous avons évoqué certaines des conditions de son utilisation.^[Voir section \ref{paragraphe:Modeles_LDA}, page \pageref{paragraphe:Modeles_LDA}]
...
Nous utilisons ici la définition *lato sensu* du caractère uni ou multimodal d'une distribution : une distribution unimodale est une distribution dotée d'un unique maximum. Une distribution multimodale est une distribution dotée de plusieurs maximums locaux.

En termes plus triviaux, la loi de densité d'une distribution unimodale n'a qu'une seule "bosse", alors que celle d'une distribution multimodale en aura plusieurs.
...
```{r Annexe-LDAMono, echo = FALSE, fig.height = 2.8, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Distributions unimodales et critère de séparation (en pointillés)"}
plot(ggarrange(
   ncol = 2, widths = c(1,1),
...
```{r Annexe-LDABi, echo = FALSE, fig.height = 2.8, out.width = "100%", dev = "jpeg", dpi = 600, fig.cap = "Distributions bimodales et critère de séparation (en pointillés)"}
plot(ggarrange(
   ncol = 2, widths = c(1,1),
   OUT_graphe_bimodale,
...
La figure suivante résume les critères descriptifs exploités dans cette étude, dont nous développerons certains éléments dans cette annexe.

![Eléments anatomiques du sporophore et caractéristiques exploitées pour son identification.](AnatomieChampis.jpg)

## Critères morphologiques

Un des critères morphologiques les plus évidents est la couleur du champignon. Toutefois, ce critère concerne plusieurs éléments\ : chapeau, stipe, hyménophore, chair et sporée.

Le nuancier suivant (figure \ref{fig:AnatNuancier}), construit à l'aide de l'échelle de Romagnési pour les teintes beige à jaune, et à partir de photographies de la littérature mycologique pour les autres teintes, illustre les principales couleurs utilisées pour caractériser nos champignons. Des variantes telles que *noircissant* ou *bleuissant* sont également utilisées afin de caractériser un virement de couleur.

![Nuancier des principales couleurs utilisées dans cette étude](anat-nuancier.png)

Un autre caractère assez évident est la forme du chapeau. La figure \ref{fig:AnatChapeaux} présente les formes de chapeaux principales pour les sporophores de champignons de morphologie agaricoïde,^[La morphologie agaricoïde correspond à la "représentation habituelle" du champignon et se caractérise par la présence d'un chapeau, d'un stipe, et d'un hyménophore à lames.] mais bien d'autres géométries existent : consoles, croûtes...

![Principales formes de chapeaux](anat-chapeaux.svg)

La marge du chapeau -- dont la figure \ref{fig:AnatMarges} illustre quelques exemples -- ainsi que sa texture sont également susceptibles de fournir quelques indices utiles à la classification.

![Principales formes de marges de chapeaux](anat-marge.svg)
...
* Insertion des lames, c'est-à-dire leur relation avec le stipe (figure \ref{fig:AnatInsertionLames}),
* Géométrie sagittale des lames, c'est-à-dire leur profil (figure \ref{fig:AnatProfilLames}),
* Géométrie radiale des lames, c'est à dire leur aspect vu de dessous (figure \ref{fig:AnatTopLames}).
...
Ces critères étant couramment utilisés dans la classification des champignons, il apparaît naturel de les inclure également dans notre étude.

![Principaux types d'insertions des lames](anat-insertionslames.svg)

![Principaux profils sagittaux d'hyménophores à lames](anat-profillames.svg)

![Principales géométries radiales d'hyménophores à lames](anat-toplames.svg)

La forme du stipe elle-même, ainsi que la texture de sa chair (fibreuse, grenue...), peuvent également fournir des indices utiles sur la famille à laquelle appartient le champignon.

Enfin, certaines structures protectrices caractéristiques, faisant partie de l'anatomie du très jeune sporophore (le *primordium*) peuvent exister à l'état de reliquats chez le spécimen adulte et constituer de précieux indices pour son identification :

* Le voile général (figure \ref{fig:AnatVoileGeneral}), qui entourait la totalité du jeune sporophore, et peut notamment subsister sous forme d'une volve ou de flocons vélaires,
* Le voile partiel (figure \ref{fig:AnatVoilePartiel}), qui recouvrait l'hyménophore du jeune sporophore, et peut subsister sous forme d'un anneau, d'une cortine ou d'une armille.

![Principaux types et modes de rupture du voile général](anat-voilegeneral.svg)

![Principaux types de voiles partiels](anat-voilepartiel.svg)

Ces critères morphologiques peuvent être complétés par des critères organoleptiques. Ces derniers peuvent, malgré les limites liées au caractère éminemment subjectif de la perception, à la "culture organoleptique" propre à chacun, ainsi qu'à la volatilité et à la fugacité de certains arômes, constituer des indices utiles dans la classification mycologique.

Nous avons donc choisi d'inclure un critère olfactif pour caractériser nos spécimens.
...
Les spécialisations que nous venons d'évoquer permettent d'affirmer que l'environnement et le substrat sur lesquels ont été prélevés le champignon peuvent constituer des indices majeurs de l'identification de celui-ci, et justifient amplement la présence d'un indicateur environnemental parmi les critères de notre classification.

Dans une moindre mesure, la période de fructification peut également constituer un indice, et a donc été incluse dans nos critères de classification.

```{r EcologieChampis, fig.align = "center", echo = FALSE, out.width = "100%", fig.cap = "Principaux modes de vie des champignons"}
include_graphics("EcosystemeChampi.jpg")
```

\newpage
# Annexe 3\ : Algorithme de génération de lot synthétique

Le principe de cet algorithme est de prendre un tableau au format csv contenant les caractéristiques typiques des macromycètes extraites de la littérature mycologique (type de sporophore, dimensions maximales du stipe, du chapeau, type de lames, couleur de sporée, etc.) et de générer un lot de données exploitable, sous forme d'une liste de spécimens, de chaque espèce donnée, possédant des caractéristiques individuelles censées être représentatives de leur espèce.
...
Cette fonction extrait tout d'abord les deux chaînes numériques *mm* et *MM*, avant d'effectuer une comparaison entre ces deux valeurs. En effet, si $mm > MM$, alors la saison de fructification devra inclure le passage de décembre à janvier, et donc générer toutes les valeurs entières de *mm* à 12, puis celles de 1 à *MM*. L'ensemble de ces valeurs est intégrée dans une chaîne de caractères via la fonction \verb|str_flatten|, avec la séparation appropriée. Cette fonction est ensuite appliquée sur l'ensemble de la variable \verb|Mois|.
...
Pour chaque variété sera ensuite créée une liste, simple copie de la ligne correspondante effectuée à partir de notre *dataframe* \verb|data_champis|. Chaque chaîne de caractères de la liste sera ensuite séparée à l'aide de la fonction définie précédemment, afin de définir des vecteurs. La sortie d'une fonction \verb|str_split| étant par définition une chaîne de caractères, toutes les valeurs dimensionnelles que nous avions précédemment identifiées comme numériques lors de l'analyse structurelle des données du lot seront forcées en tant que numériques via la fonction \verb|as.numeric|.
...
* \verb|tidyverse|, collection de bibliothèques spécialisées dans le domaine de la *data science*,[@R-tidyverse]
* \verb|DiceDesign|, bibliothèque spécialisée dans la création de plans d'expériences hypercubiques,[@R-DiceDesign]
* \verb|DiceEval|, bibliothèque spécialisée dans la modélisation des résultats de plans d'expériences hypercubiques,[@R-DiceEval]
* \verb|caret|, collection d'outils dédiés à l'apprentissage machine.[@R-caret]
* \verb|twinning|, outils dédiés à la génération de jeux de données d'entraînement, optimisation, validation équilibrés.[@R-twinning]
...
La création du jeu d'évaluation s'effectue en deux étapes. La première est la définition des rapports des dichotomies entre jeux d'entraînement et optimisation d'une part, et d'évaluation d'autre part. Cette définition implique l'évaluation du nombre d'individus, le calcul du nombre de coefficients $p$, puis du rapport de dichotomie $f = \sqrt(p) +1$.^[cf. section \ref{chapitre:split}, page \pageref{paragraphe:split_ratio}.]
...
La seconde partie consiste à effectuer la scission proprement dite. Cette scission implique la définition d'une liste d'index, de fraction $1:f$ du nombre d'individus, qui servira via inclusion (jeu d'évaluation) ou exclusion (jeu d'apprentissage et d'évaluation) booléennes des lignes correspondantes, à constituer chaque jeu de données. La fonction \verb|set.seed| assure la reproductibilité.
\small
```{r, eval = FALSE}
set.seed(7)
...
pander::pander(BI_fit_rpart_cp_resultats, caption = "Tableau des résultats de l'entraînement de rpart")
```
\normalsize
...
```{r, echo = FALSE, fig.height = 3, fig.width = 5, dev = "jpeg", dpi = 600, fig.cap = "Graphique des performances de rpart"}
plot(BI_fit_rpart_cp_graphe)
```
...
```{r, echo = FALSE}
pander::pander(BI_grid_Rborist, caption = "Plan d'expériences d'entraînement et optimisation du modèle Rborist")
```

L'entraînement du modèle se déroule de la même façon que pour le modèle rpart.^[cf. section \ref{chapitre:code-CART}, page \pageref{paragraphe:train-CART}]
...
Ces résultats permettent notamment de modéliser les performances sur la totalité de l'espace expérimental des hyperparamètres. A cette fin, l'algorithme est chargé de générer l'ensemble des couples de valeurs $(X_{1},X_{2})$ possibles, à l'aide de la fonction \verb|expand.grid|, avant de calculer la valeur $J_{w}$ correspondant à chaque couple de points.

L'évaluation de la précision de la modélisation quadratique se base sur la juxtaposition des prédictions suivant le modèle quadratique à côté des valeurs expérimentales, puis le calcul de l'erreur absolue moyenne (MAE).

\small
```{r, eval = FALSE}
BI_pred_Rborist <- expand.grid(CodBI_fit_Rborist_resultats[,c("X1","X2")]) %>%
...
Les performances du modèle peuvent être évaluées, en termes d'efficience calculatoire, par son temps d’exécution. Un moyen de mesurer le temps d'exécution de n'importe quelle portion de code est de mesurer l'heure de début et de fin d'exécution du code à l'aide de \verb|Sys.time|, puis d'en calculer la différence via la fonction \verb|difftime|.

Le code exécuté correspond ici à l'entraînement du modèle (cf. *supra*) et à la prédiction sur le lot d'évaluation, enregistrée dans un objet dédié.
...
\newpage
# Annexe 5\ : Langage de balisage Rmarkdown  {#chapitre:Rmarkdown}
...
[@fisher_use_1936;@anderson_r_1996] 
La LDA est basée sur la construction de l'hyperplan de projection permettant de 
maximiser la distance entre les moyennes projetées des différentes classes et de 
minimiser la variance intraclasse (voir figure \ref{fig:RMD-Principe-LDA}).
[@hastie_elements_2016] 
La LDA peut être utilisée à fins de classification, mais aussi pour effectuer des 
réductions de dimensionnalité ou encore afin de faciliter l'interprétation de 
...
\newpage
Les commandes utilisées dans le bloc précédent sont les suivantes :
...
Le bloc suivant n'appelle pas de commentaires particuliers, étant purement textuel :
\small
````{verbatim}
\paragraph*{}
...
Nous pouvons ensuite introduire des notations mathématiques, toujours au format Latex, ce qui permet d'exploiter un des points forts de ce langage, qui propose un rendu de haute qualité des expression mathématiques :
...
d_{i} = \overline{x_{i, a}} - \overline{x_{i, b}}$$
$\overline{x_{i, a}}$ et $\overline{x_{i, b}}$ étant les moyennes respectives 
de chaque caractéristique $x_{i}$ pour les groupes (espèces) $a$ et $b$, et :
...
L'insertion de tableaux sous Rmarkdown s'effectue grâce à la fonction \verb|kable|, et les paramètres de style peuvent être configurés plus finement à l'aide de la fonction \verb|kable_styling| apportée par la librairie \verb|kableExtra|. Nous avons notamment ajouté des tons bicolores pour le fond des lignes du tableau (\verb|striped|), et bloqué la position des tableaux vis-à-vis du texte (\verb|hold_position|).

Le code se poursuit par des éléments textuels et mathématiques n'appelant pas à commentaires particuliers.
...
d_{4}=S_{41}\lambda_{1}+S_{42}\lambda_{2}+S_{43}\lambda_{3}+S_{44}\lambda_{4}\\
\end{array} 
\Rightarrow \mathbf{S.\boldsymbol{\lambda} = D \Leftrightarrow{} 
\boldsymbol{\lambda} = S^{-1}.D}
\right.$$
...
Le dernier bloc de code présenté dans cette annexe permet d'insérer des graphiques. De même que pour les tables et les valeurs numériques, Rmarkdown s'illustre ici par ainsi sa grande souplesse qui permet la génération automatique de fichiers texte intégrant graphiques, tableaux et données numériques ou textuelles calculées et mises à jour automatiquement.
...
La fin du code n'appelle pas à commentaires particuliers. La table des matières s’insérera automatiquement dans la toute dernière page, conformément au préambule, et avant les annexes éventuelles.
