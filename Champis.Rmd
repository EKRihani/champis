...
\paragraph*{}
L'analyse discriminante linéaire est une méthode ayant été proposée par Ronald Fisher en 1936[@fisher_use_1936;@anderson_r_1996] pour résoudre des problèmes de classification taxonomique dans le domaine de la botanique.\footnote{Cette étude, proposant une méthode de classification des variétés *Iris setosa*, *Iris virginica* et *Iris versicolor* est par ailleurs à l'origine du célèbre jeu de données *Iris*.} La LDA est basée sur la construction de l'hyperplan de projection permettant de minimiser la variance intraclasse et maximise la distance entre les moyennes projetées des différentes classes. Cet hyperplan de projection peut être utilisé pour la classification, mais aussi pour la réduction de dimensionalité ou encore l'interprétation de l'importance de certaines caractéristiques.
\paragraph*{}
...
avec $S$ la variance et $S_{pq}$ la somme des produits des caractéristiques pour chaque combinaison de paramètres $p$ et $q$.
\paragraph*{}
...
$$\frac{d}{d\lambda{}} \frac{D^2}{S} = 0 \Leftrightarrow{} \left(2S\frac{\partial{}D}{\partial{}\lambda{}} - D \frac{\partial{}S}{\partial{}\lambda{}} \right) = 0 \Leftrightarrow{} \frac{D}{2S} \frac{\partial{}S}{\partial{}\lambda{}} = \frac{\partial{}D}{\partial{}\lambda{}} $$

Les coefficients requis sont donc les solutions du système $\mathbf{S}.\mathbf{\lambda} = \mathbf{D}$ avec $\mathbf{S}$ la matrice des $S_{pq}$, $\mathbf{D}$ le vecteur des différences des moyennes $d_{i}$ et $\mathbf{\lambda{}}$ le vecteur des coefficients $\lambda_{i}$.
\paragraph*{}
