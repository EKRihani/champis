...
Les performances maximales sont atteintes pour *cp =* `r best_rpartcostgrid['cp']` et *Cost = * `r best_rpartcostgrid['Cost']`.
...
Les étapes préliminaires de l'optimisation du modèle se sont avérées prometteuses : même avec un très faible nombre d'arbres (*n* = 6), la spécificité requise était déjà atteinte à plusieurs reprises lors de l'optimisation uniparamètre. L'optimisation de la totalité des hyperparamètres (*min.node.size* = `r best_rangergrid$min.node.size`, *mtry* = `r best_rangergrid$mtry` et *splitrule* = `r best_rangergrid$splitrule`) a donné d'excellents résultats.
...
plot(ggarrange(
   ncol = 2, widths = c(8,7),
   fit_Rborist_spec_graphe,
   fit_Rborist_sens_graphe + ylab(NULL)
   )
)
```
...
\paragraph*{}
Avec des paramètres optimaux (*predFixed* = `r best_Rboristgrid$predFixed` et *minNode* = `r best_Rboristgrid$minNode`), la performance est estimée à :
\FloatBarrier

```{r, echo = FALSE}
kable(fit_Rborist_best_results[1:5], caption = "Performances du modèle Rborist (hyperparamètres optimaux)")
```
\FloatBarrier
\paragraph*{}
