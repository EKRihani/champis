...
\paragraph*{} \label{paragraphe:split}
...
\paragraph*{}
Le lot de données d'origine contient `r EDA_n_champis` variétés de champignons, caractérisées par `r EDA_n_cols` propriétés morphologiques ou environnementales. La structure de ce lot de données est résumé dans le tableau \@ref(tab:EDA-structure).
\paragraph*{}
Le lot de données d'origine a été découpé en un jeu d'apprentissage/optimisation et un jeu de données d'évaluation, avec un rapport `r EDA_split_facteur`:1, conformément aux principes mentionnés dans nos développements précédents.\footnote{cf. section \ref{chapitre:split}, page \pageref{paragraphe:split}.}
\paragraph*{}
...
* Modélisation des performances en fonction des hyperparamètres, via un modèle quadratique avec interactions\footnotemark[\value{footnote}]
