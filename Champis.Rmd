...
\paragraph*{}
L'application sur les espèces *iris versicolor* et *iris setosa* nous donne :
...
```{r, echo = FALSE}
kable(iris_prods, caption = "Produits des différences à la moyenne des 4 paramètres d'Iris setosa et versicolor (S)") %>%
   kable_styling(latex_options = c("striped", "hold_position"),full_width = F)
```

\paragraph*{}
La maximisation du rapport entre carrés des distances des moyennes interclasses et variances intraclasses revient à maximiser $D^{2}/S$ pour chaque coefficient $\lambda{}_{i}$ soit, par dérivation pour chacun des $\lambda{}_{i}$ :
...
\paragraph*{}
La valeur absolue des coefficients $\lambda{}_{i}$ calculés précédemment indique la pondération de chaque caractère dimensionnel dans l'indice synthétique $X$ permettant d'obtenir une séparation optimale, ainsi que l'illustrent les figures \@ref(fig:LDA-MinMax) et \@ref(fig:LDA-Separation).
...
\FloatBarrier
\paragraph*{}
Nous pouvons conclure cette présentation de la LDA en soulignant ses limites, illustrées par certains développements de cette section :

* Les modèles LDA ne sont adaptés qu'à des données quantitatives ou qualitatives ordinales,
* La séparation entre les différentes classes est basée sur la construction d'un indice synthétique purement linéaire : dans l'hyperplan de projection, la séparation entre "territoires" des différentes classes sera toujours une droite,
...
Les arbres de décision sont l'une des formes les plus populaires de représentation des connaissances utilisées dans le *data mining*.[@wu_top_2009] Une des raisons de cette popularité est que les résultats obtenus sont généralement considérés comme performants, et que l'apprentissage comme les prédictions sont aisément compréhensibles, contrairement à certaines approches dont la complexité peut les rapprocher de modèles assimilables à des boîtes noires.[@rokach_data_2015]
\paragraph*{}
Les arbres de décision peuvent aussi bien être utilisés pour des problèmes de classification (prédictions de classes) que de régression (prédiction de valeurs numériques). Les arbres de décisions sont des structures hiérarchiques, séquentielles, composées de n\oe{}uds reliés par des branches. Les n\oe{}uds sont conceptuellement de deux types :
...
* Les n\oe{}uds terminaux, dépourvus de descendants, qualifiés de feuilles, responsables de la prédiction : classe (arbre de classification) ou valeur numérique (arbre de régression).

Le fonctionnement de l'arbre de classification se base donc sur une succession de tests -- les plus importants se situant près de la racine -- dont les résultats permettent d'avancer dans la hiérarchie de l'arbre, étape après étape, afin d'aboutir à une décision finale, dans une démarche pouvant mimer certains aspects du raisonnement humain.[@rokach_data_2015]

\paragraph*{}
Les tests des arbres de décision peuvent être :

* Univariés, typiquement un test d'inégalité de forme $x_{i} < S$ avec $x_{i}$ la caractéristique à mesurée et $S$ un seuil permettant la décision. Ce test d'inégalité aboutit typiquement à une bifurcation, mais certains modèles peuvent regrouper plusieurs tests sur une caractéristique pour aboutir à une multifurcation.
...
\paragraph*{}
Les modèles d'arbres de classification peuvent utiliser des tests purement univariés (ID3, C4.5, CHAID...), purement obliques (arbres *perceptron*) ou autoriser les deux types de tests (arbres CART : *Classification And Regression Trees*).[@loh_fifty_2014]
\paragraph*{}
L'algorithme le plus utilisé pour la conception des arbres de classification est l'induction du haut vers le bas, basée sur l'algorithme classique dit "diviser pour régner" (*divide and conquer*), qui consiste à découper le problème de base en sous-problèmes, puis à résoudre ces sous-problèmes avant de les combiner en une solution globale.
\paragraph*{}
En pratique, un n\oe{}ud racine est tout d'abord créé, auquel sera associé la totalité du jeu d'apprentissage. En partant de ce n\oe{}ud initial sera appliqué un algorithme récursif qui essaie de diviser ces données, à chaque étape duquel les n\oe{}ds créés peuvent soit être marqués comme feuille (auquel cas l'induction est terminée), soit comme n\oe{}u interne (auquel cas l'algorithme se poursuit et l'arbre continuera de grandir). A chaque étape de cet algorithme, la taille du jeu de données disponible pour chaque n\oe{}ud se réduit, et la probabilité que le n\oe{}ud devienne une feuille augmente.
\paragraph*{}
...
* De l'inhomogénéité des classes peuplant chaque n\oe{}ud postérieur à la scission pour les problèmes de classification,
* De la somme de la valeur absolue des résidus ou de leurs carrés pour les problèmes de régression.

Pour le modèle CART qui sera utilisé dans la classification binaire cette étude, cette inhomogénéité est évaluée par le coefficient de Gini :[@wu_top_2009]
$$G(t) = 1- p(t)^{2} - \left(1- p(t) \right)^{2}$$
avec $p(t)$ la fréquence relative d'une des deux classes au sein du n\oe{}ud considéré.
\paragraph*{}
Enfin, les arbres obtenus peuvent être rationalisés à l'aide d'un algorithme d'élagage (*pruning*) visant à réduire la complexité de l'arbre en l'analysant des feuilles à la racine, et en évaluant si un n\oe{}ud intermédiaire peut avantageusement être remplacé par une feuille. Différents critères d'optimisation existe, parmi lesquels nous pouvons citer l'élagage coût-complexité[@kretowski_evolutionary_2019], exploité par le modèle CART, et qui vise à minimiser le facteur coût-complexité $CC(T)$ de l'arbre $T$ :
