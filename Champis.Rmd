...
\paragraph*{}
Ces différents éléments anatomiques, ainsi que l'environnement dans lequel prospère le champignon, représentent autant de caractéristiques permettant l'identification d'une espèce, et qui seront exploitées dans cette étude (voir figure \ref{fig:Anatomie-Champis}).\footnote{Voir également annexes, page \pageref{chapitre:annexemyco} et suivantes.}
\paragraph*{}
...
\paragraph*{}
```{r Lots-Dispersion2D, echo = FALSE, fig.height = 3.2, dev = "jpeg", dpi = 600, fig.cap = paste0("Nuages de points de 2 paramètres de taille (Ls et Dc), sans dispersion (à gauche) et avec dispersion (à droite), pour ", n_reduit, " champignons")}
plot(ggarrange(
   ncol = 2,
...
avec $\alpha{}$ un hyperparamètre de complexité\footnote{cf. annexes, section \ref{paragraphe:train-CART}, page \pageref{paragraphe:train-CART} et suivantes} strictement positif évalué expérimentalement.\footnote{cf. section \ref{chapitre:doe}, page \pageref{chapitre:doe}}
\paragraph*{}
...
\begin{figure}
   ...
   \caption{Structure arborescente complète proposée par le modèle CART pour la classification des champignons}
...
* Apparition, grâce notamment aux avancées de la génomique, de problèmes dits de type *grand p, petit n*, avec un grand nombre de prédicteurs $p$ pour un petit nombre d’observations $n$, qui mettent en échec la vision classique de parcimonie d’un modèle unique pour un problème unique,\footnote{En l'espèce, les problèmes posés par la génomique imposent souvent d'analyser des dizaines de milliers de gènes (\emph{p}), sur quelques dizaines ou centaines de patients (\emph{n}).}
...
\begin{figure}
   \centering
   \includegraphics[width=0.495\linewidth]{BI_Arbre_Rpartcost_Sens}
   \hfill
   \includegraphics[width=0.495\linewidth]{BI_Arbre_Rpartcost_Spec}
   \caption{Arbres simplifiés, optimisant la sensibilité (à g.) ou la spécificité (à d.)}
   \label{fig:ArbresRpartCost}
\end{figure}
<!-- NE PAS OUBLIER DE CAVIARDER LES VALEURS ULTIMES DE L'ARBRE PRUDENT -->
\paragraph*{}
Les arbres de classification les plus simples obtenus (figure \ref{fig:ArbresRpartCost}) avec des valeurs d'hyperparamètre *Cost* se situant aux limites de notre espace expérimental présentent ainsi des structures et des critères assez différents, selon que l'on optimise la sensibilité pour obtenir un arbre "conservateur", ou la spécificité pour obtenir un arbre "optimiste" (voire suicidaire).\footnote{Les n\oe{}uds aboutissant à des feuilles de type \emph{100\% comestibles} ont été censurés.}
\paragraph*{}
Nous pouvons notamment remarquer la forte prévalence de n\oe{}uds intermédiaires dotés de valeurs *Rejeter* ou *Conserver*, selon que l'accent ait été porté sur la sensibilité (tendance au rejet) ou la spécificité (tendance à la consommation).
\paragraph*{}
...
\paragraph*{}
Tous les modèles ont été entraînés sur le jeu de données d'apprentissage, après application des hyperparamètres optimaux obtenus précédemment par modélisation des performances via un modèle quadratique avec interactions.\footnote{cf. section \ref{chapitre:BI-RF}, page \pageref{chapitre:BI-RF}} Les performances de nos modèles face au jeu de données d'évaluation, auquel ils n'ont encore jamais été exposés, seront évaluées avec le même critère que précédemment\ : $J_{w} \geq `r Jw_min`$.\footnote{cf. section \ref{chapitre:split_methodes}, page \pageref{chapitre:split_methodes}}
...
```{r, echo = FALSE, fig.height = 4, dev = "jpeg", dpi = 600, fig.cap = "Performances du modèle Ranger dans une classification par familles, en fonction de ses hyperparamètres (algorithme de scission\ : Gini à gauche, extratrees à droite)"}
plot(ggarrange(
   ncol = 2,
...
\paragraph*{}
Comme précédemment, nous pouvons proposer pour ce modèle la modélisation quadratique suivante\ :\footnote{cf. section \ref{paragraphe:ModelBIRanger}, page \pageref{paragraphe:ModelBIRanger}} \label{paragraphe:ModelMULFAMRanger}
$$ Y = b_{0} + b_{1}.X_{1} + b_{2}.X_{2} + b_{3}.X_{3} + b_{12}.X_{1}.X_{2} + b_{23}.X_{2}.X_{3} + b_{13}.X_{1}.X_{3} + b_{11}.X_{1}^{2} + b_{22}.X_{2}^{2}$$
...
```{r, echo = FALSE, fig.height = 4, dev = "jpeg", dpi = 600, fig.cap = "Performances du modèle Ranger dans une classification par espèces, en fonction de ses hyperparamètres (algorithme de scission\ : Gini à gauche, extratrees à droite)"}
plot(ggarrange(
   ncol = 2,
...
\FloatBarrier
\paragraph*{}
Le calcul numérique des coefficients de la modélisation quadratique du kappa en fonction des hyperparamètres réduits de *mtry* ($X_{1}$, nombre de caractéristiques à séparer à chaque n\oe{}ud), *min.node.size* ($X_{2}$, taille minimale de n\oe{}ud) et *splitrule* ($X_{3}$, règle de séparation) nous permet de confirmer l'impact prédominant de l'hyperparamètre *mtry* et de calculer les estimations des effets suivantes\ :\footnote{cf. section \ref{paragraphe:ModelBIRanger}, page \pageref{paragraphe:ModelBIRanger} et section \ref{paragraphe:ModelMULFAMRanger}, page \pageref{paragraphe:ModelMULFAMRanger}}
\paragraph*{}
:::::: {.cols data-latex=""}
...
\paragraph*{}
Comme précédemment, le calcul numérique permet d'estimer les effets des hyperparamètres et de confirmer l'impact de *predFixed* ($X_{1}$)\ :\footnote{cf. section \ref{paragraphe:ModelBIRborist}, page \pageref{paragraphe:ModelBIRborist} et section \ref{paragraphe:ModelMULFAMRborist}, page \pageref{paragraphe:ModelMULFAMRborist}}
\paragraph*{}
:::::: {.cols data-latex=""}
...
\paragraph*{}
Comme évoqué dans nos développements précédents, la génération des données quantitatives est aléatoire, mais obéit à des lois statistiques définies.\footnote{Voir section \ref{chapitre:lotsynth}, page \pageref{chapitre:lotsynth}}
\paragraph*{}
...
\FloatBarrier
Un autre caractère assez évident est la forme du chapeau. La figure \ref{fig:AnatChapeaux} présente les formes de chapeaux principales pour les sporophores de champignons de morphologie agaricoïde,\footnote{La morphologie agaricoïde correspond à la "représentation habituelle" du champignon et se caractérise par la présence d'un chapeau, d'un stipe, et d'un hyménophore à lames.} mais bien d'autres géométries existent : consoles, croûtes...
\FloatBarrier
\begin{figure}
   \centering
...
\begin{figure}
   \centering
   \includegraphics[width=\linewidth]{anat-marge}
  \caption{Principales formes de marges de chapeaux}
  \label{fig:AnatMarges}
\end{figure}
...
\begin{figure}
  ...
  \caption{Principaux types d'insertions des lames}
