...
load("EKR-Champis-Iris.RData")  # Chargement des exemples Iris
...
\paragraph*{}
En pratique, ce domaine se consacre à la génération d’algorithmes autonomes capables d’améliorer leurs performances prédictives par exposition à un lot de données et de prédire par inférence la meilleure décision ou réponse à une question.[@michael_machine_2019]

## Types d'apprentissage machine

L’apprentissage machine peut se dérouler suivant un certain nombre de paradigmes, concernant en particulier la stratégie d’apprentissage -- définie à partir du type de données disponibles -- parmi lesquels nous pouvons notamment citer\ :[@michael_machine_2019;@amr_hands-machine_2020]
...
\paragraph*{}
Le modèle CART le plus simple propose des performances excellentes sur une large plage d'hyperparamètres, remplissant notre critère $Jw \geq `r Jw_min`$, avec les résultats suivants\ :
$$\left \{
\begin{array}{l}
...
\paragraph*{}
Les performances du modèle rpartCost, sont excellentes, dépassant celles de notre modèle précédent, et remplissant donc notre critère concernant l'index J de Youden pondéré minimal à atteindre. L'équilibre entre sensibilité et spécificité est à souligner, malgré la pondération `r Jw_ratio`:1 de ces critères dans notre indice synthétique.
\paragraph*{}
...
Les modèles d'arbres de classification sont réputés pour être particulièrement adaptés aux problèmes de classification avec variables quantitatives et surtout qualitatives, et ont pu s'illustrer dans cette tâche de classification en fournissant des résultats excellents ($Spe \geq `r round(min(BI_fit_c50tree_resultats["Spec"], 
    ...
    BI_fit_rpart_cp_resultats[which.max(BI_fit_rpart_cp_resultats[,"Jw"]),"Sens"]),4)`$), dépassant tous les exigences imposées par le critère de performance que nous avions défini pour les classifieurs binaires de notre étude.Je 
\paragraph*{}
...
```{r, echo = FALSE, fig.height = 4, dev = "jpeg", dpi = 720, fig.cap = "Sensibilité (à gauche) et spécificité (à droite) du modèle Rborist en classification binaire, en fonction de ses deux hyperparamètres (interpolation Kriging quadratique, points expérimentaux encadrés en noir)"}
plot(ggarrange(
   ncol = 2,
...
\paragraph*{}
Le lot résultant est exporté sous format csv, compressé dans un fichier zip, et prêt à être exploité dans les autres parties de cette étude.
\small
```{r, eval = FALSE}
...
\paragraph*{}
L'étape finale consiste à nettoyer l'environnement en supprimant tous les objets de type *champX* et *lotX*.
\small
```{r, eval = FALSE}
for (n in 1:n_especes){
  ordre_rm1 <- paste0("rm(champ",n, ")")
  ordre_rm2 <- paste0("rm(lot",n, ")")
...
Dans le cadre des classifications binaire et multiclasse, nous ôterons respectivement le nom de chaque spécimen (variable *Nom*), et sa comestiblité (variable *Type*), ces variables étant censées être inconnues du modèle prédictif et ne représentant pas la caractéristique à prédire.
