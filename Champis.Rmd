...
\FloatBarrier
\paragraph*{}
Les performances maximales seront ici atteintes pour $cp = `r BI_best_rpartcostgrid["cp"]`$ et $Cost = `r BI_best_rpartcostgrid["Cost"]`$, pour lesquelles nous obtenons :

$$\left \{
\begin{array}{l}
J_{w_{max}} = `r round(max(BI_fit_rpartcost_resultats["Jw"]),4)` \\
Spe_{Jw_{max}} = `r round(BI_fit_rpartcost_resultats[which.max(BI_fit_rpartcost_resultats[,"Jw"]),"Spec"],4)` \\
Sen_{Jw_{max}} = `r round(BI_fit_rpartcost_resultats[which.max(BI_fit_rpartcost_resultats[,"Jw"]),"Sens"],4)` \\
\end{array}
\right.$$
