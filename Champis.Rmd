...
\paragraph*{}
Le paramètre *lambda* du modèle pda impacte sa spécificité de façon marginale, avec des lambdas faibles donnant une légère amélioration des résultats ($Spec_{max} =$ `r round(max(fit_pda_lambda_results["Spec"]))`).
\paragraph*{}
...
```{r, echo = FALSE, fig.height = 2.5, fig.cap = "Spécificité du modèle Rborist"}
plot(ggarrange(
   ncol = 2,
...
```{r, echo = FALSE}
kable(fit_Rborist_pred_results[1:5, 1:5], digits = 4, caption = "Performances du modèle Rborist (réglage de predFixed)")
```
...
```{r, echo = FALSE}
kable(fit_Rborist_best_results[1:5], caption = "Performance du modèle Rborist (hyperparamètres optimaux)")
```
\FloatBarrier
\paragraph*{}
...
La précision finale est égale à `r round(CM_ranger_final$overall["Accuracy"], 4)`, avec un intervalle de confiance à 95% de [`r round(CM_ranger_final$overall["AccuracyLower"], 4)` ; `r round(CM_ranger_final$overall["AccuracyUpper"], 4)`]. La forêt aléatoire de type Ranger a donné d'excellents résultats, en un temps très contenu (`r time_ranger` min), preuve de sa grande efficience calculatoire.
\paragraph*{}
La forêt aléatoire de type Rborist a donné des résultats similaires, avec une précision finale égale à `r CM_Rborist_final$overall["Accuracy"]`, avec un intervalle de confiance à 95% de [`r round(CM_Rborist_final$overall["AccuracyLower"], 4)` ; `r round(CM_Rborist_final$overall["AccuracyUpper"], 4)`]. Le modèle Rborist, donnant des résultats identiques à Ranger, s'est avéré extrêmement efficient sur le plan calculatoire (`r time_Rborist` min).
