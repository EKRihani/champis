...
plot(ggarrange(
   ncol = 3,
   study_distrib_Chapeau.Diametre + ggtitle("") + ylab("") + scale_y_continuous(labels = NULL),
   study_distrib_Pied.Hauteur + ggtitle("") + ylab("") + scale_y_continuous(labels = NULL),
   study_distrib_Pied.Largeur + ggtitle("") + ylab("") + scale_y_continuous(labels = NULL)
   )
)
```
...
plot(ggarrange(
   ncol = 3,
   study_distrib_Chapeau.Diametre + ggtitle("") + ylab("") + scale_y_log10(labels = NULL),
   study_distrib_Pied.Hauteur + ggtitle("") + ylab("") + scale_y_log10(labels = NULL),
   study_distrib_Pied.Largeur + ggtitle("") + ylab("") + scale_y_log10(labels = NULL)
   )
)
```
...
\paragraph*{}
Les performances du modèle rpartCost sont excellentes, dépassant celles de notre modèle précédent, au prix d'une efficience calculatoire amoindrie ($t_{moy} = `r BI_temps_rpartcost`$ min, $n = `r nrow(BI_grid_rpartcost)`$ itérations). Ce modèle remplit pleinement notre critère concernant l'index J de Youden pondéré minimal à atteindre. L'équilibre entre sensibilité et spécificité est à souligner, malgré la pondération `r Jw_ratio`:1 de ces critères dans notre indice synthétique.
\paragraph*{}
...
\right.$$
\paragraph*{}
Bien que ne disposant d'aucun hyperparamètre, ce modèle a donné d'excellents résultats, remplissant aisément le critère $J_w \geq `r Jw_min`$, sans aucune optimisation nécessaire. En outre, ce modèle s'illustre par une sensibilité -- c'est à dire une capacité à rejeter les champignons non comestibles -- particulièrement élevée.
\paragraph*{}
Le temps de calcul est relativement long ($t = `r BI_temps_c50tree`$ min), signe d'une efficience calculatoire modeste, mais cette remarque est à modérer en raison des gains que peut apporter un modèle s'affranchissant totalement de la phase d'optimisation des hyperparamètres :
...
\paragraph*{}
Les modèles d'arbres de classification sont réputés pour être particulièrement adaptés aux problèmes de classification avec variables quantitatives et surtout qualitatives, et ont pu s'illustrer dans cette tâche de classification en fournissant des résultats excellents. 
\paragraph*{}
En effet, la totalité des modèles d'arbres testés ici dépassent les exigences imposées par le critère de performance que nous avions défini pour les classifieurs binaires de notre étude.
\paragraph*{}
...
\newpage
# Annexe 4\ : Langage de balisage Rmarkdown
\paragraph*{}
Rmarkdown est un langage de balisage particulièrement puissant, permettant de combiner les possibilités de R et de Latex. Ce langage a été intégralement utilisé pour rédiger cette étude, et cette annexe a pour but d'esquisser les possibilités offertes par ce langage, en reprenant succinctement la partie décrivant la LDA.
\paragraph*{}
...
\normalsize
\paragraph*{}
Cette partie présente aussi une figure au format vectoriel, enregistrée sous le nom de fichier *LDA.pdf*, ainsi que des références bibliographiques, enregistrées dans un fichier au format Bibtex, sous le nom *Champis-SourceBiblio.bib*, dont le bloc suivant reprend le contenu :
