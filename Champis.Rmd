...
\paragraph*{}
L'étude des distributions dans un espace bidmensionnel (figure\ \ref{fig:EDA-bidistrib}, p.\ \pageref{fig:EDA-bidistrib}) permet déjà de dégager des associations de caractéristiques d'intérêt, ici représentées sous formes de zones unicolores où un type de champignons (comestible ou non) dominera, ce qui permet déjà d'élaborer des stratégies de classification basiques\footnote{cf. section \ref{chapitre:naif}, page \pageref{chapitre:naif}}.
\paragraph*{}
Ainsi, il est déjà possible de constater, d'après le nuage de points en haut à droite de la figure \ref{fig:EDA-bidistrib} que dans le lot de données, *tous* les champignons pour lesquels $cap.diameter \geq 0.1$ seront comestibles, et de même pour $stem.height \geq 20$.

```{r EDA-bidistrib, echo = FALSE, fig.height = 6.5, fig.cap = "Distributions bi-critères pour 4 critères d'intérêt"}
paires_graphes
```
