...
\paragraph*{}
L'interprétation graphique de l'exploration des hyperparamètres (figure \ref{fig:MULFAM2DRborist}) met en évidence, outre la vaste plage d'hyperparamètres permettant d'obtenir une précision maximale, une prévalence de l'hyperparamètre associé au nombre de prédicteurs (*predFixed*) dans l'efficacité de cet algorithme.
\paragraph*{}
...
\paragraph*{}
L'interprétation des graphiques obtenus suite à l'exploration de l'espace expérimental des hyperparamètres de ce modèle laisse entrevoir de bonnes performances sur une large plage de ce domaine, avec un effet nettement prédominant de *mtry* sur les performances globales du modèle.
...
\paragraph*{}
Comme précédemment,\footnote{cf. section \ref{paragraphe:ModelBIRborist}, page \pageref{paragraphe:ModelBIRborist} et section \ref{paragraphe:ModelMULFAMRborist}, page \pageref{paragraphe:ModelMULFAMRborist}} le calcul numérique permet d'estimer les effets des hyperparamètres et de confirmer l'impact de *predFixed* ($X_{1}$)\ :
\paragraph*{}
:::::: {.cols data-latex=""}
...
\paragraph*{}
La précision finale de Rborist est ainsi égale à `r round(MULESP_CM_Rborist_final$overall["Accuracy"],4)`, avec un intervalle de confiance à 95% de [`r round(MULESP_CM_Rborist_final$overall["AccuracyLower"], 4)` ; `r round(MULESP_CM_Rborist_final$overall["AccuracyUpper"], 4)`]. Le modèle Rborist, donnant des résultats proches à Ranger, s'est avéré un peu moins efficient sur le plan calculatoire (`r MULESP_temps_Rborist` min).
\paragraph*{}
...
La marge du chapeau (fig. \ref{fig:AnatMarges}) et sa texture sont également susceptibles de fournir quelques indices utiles à la classification.
