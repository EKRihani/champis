...
```{r BI-CM, out.width = "100%", echo = FALSE}
...
```{r out.width = "100%", echo = FALSE, fig.pos="H"}
...
```{r FAM-CM, out.width = "40%", echo = FALSE}
kable(MULFAM_CM_ranger_final$table, caption = "Matrice de confusion du modèle Ranger") %>%
   kable_styling(latex_options = c("striped", "hold_position"),full_width = F, font_size = 6) %>%
  row_spec(0, angle = -90)
```
...
\paragraph*{}
Le chargement des données s'effectue de la même façon que lors des sections précédentes. Dans le cadre d'une classification binaire, nous définissons arbitrairement, à l'aide de la fonction *relevel*, la classe *"toxique"* comme étant la valeur positive. Cette définition n'est pas nécessaire pour les classifieurs multiclasses.
...
La première étape est de définir l'indice J de Youden,\footnote{cf. section \ref{chapitre:perf}, page \pageref{chapitre:perf}} et les pondérations respectives de la sensibilité et de la spécificité, par :
\small
```{r, eval = FALSE}
BI_w <- 10
...
L'algorithme génère également un graphique synthétisant les performances du modèle (sensibilité, spécificité, $J_{w}$) en fonction de son hyperparamètre :
...
plot(ggarrange(
   ...
   Code_Rborist_graphe_tiles + theme(legend.position = "none"),
   Code_Rborist_graphe_raster + ylab(NULL),
   Code_Rborist_graphe_full + ylab(NULL) + theme(legend.position = "none")))
```

\FloatBarrier

*Finir prédictions de performances...*
