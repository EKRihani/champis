...
```{r, echo = FALSE, fig.height = 3, fig.cap = "Spécificité de rpartCost en fonction des valeurs de complexité et de coût"}
plot(ggarrange(
   ncol = 2,
...
De manière assez surprenante, bien que ne disposant d'aucun hyperparamètre, ce modèle a donné d'excellents résultats sans optimisation nécessaire, avec de très hautes sensibilité et spécificité. Toutefois, le modèle C.50 tree n'a pas rempli l'objectif posé par le critère *Spec* = 1.
\FloatBarrier
\newpage
...
\paragraph*{}
Les modèles ayant atteint le critères de spécificité ($Spec = 1$) lors de l'étape d'optimisation ont été choisis pour l'évaluation. Les deux modèles choisis sont deux forêts aléatoires
...
1. La spécificité *doit* être égale à 1.
2. La sensibilité doit être la plus haute possible.
...
\paragraph*{}
La forêt aléatoire de type Rborist a donné des résultats similaires, avec une précision finale égale à `r CM_Rborist_final$overall["Accuracy"]`, avec un intervalle de confiance à 95% de [`r round(CM_Rborist_final$overall["AccuracyLower"], 4)` ; `r round(CM_Rborist_final$overall["AccuracyUpper"], 4)`]. Le modèle Rborist, donnant des résultats identiques à Ranger, s'est avéré sensiblement plus lent (`r time_Rborist` min), bien qu'ayant été lancé avec un nombre d'arbres moindre (*n* = 3 vs 10). [RELIER AU CODE]
