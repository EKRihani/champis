...
\paragraph*{}
La précision de cette modélisation peut être évaluée par l'erreur absolue moyenne :
...
```{r, echo = FALSE, fig.height = 4, dev = "jpeg", dpi = 720, fig.width = 5.2, fig.cap = paste0("Performances (indice de Youden pondéré ", Jw_ratio,":1) de rpartCost en classification binaire, en fonction des hyperparamètres réduits de complexité X1 et de coût X2 (interpolation Kriging quadratique, points expérimentaux encadrés en noir)")}
...
\end{array}
\right. \\
\quad MAE = `r signif(BI_MAE_rpartcost,4)`
\end{array}$$
\end{minipage}
\begin{minipage}{0.4 \textwidth}
$$\left \{
...
\end{array}
\right. \\
\quad MAE = `r signif(BI_MAE_Rborist,4)`
\end{array}$$
\end{minipage}
\begin{minipage}{0.4 \textwidth}
$$\left \{
...
\paragraph*{}
Ce modèle quadratique avec interactions modélise bien les données expérimentales, avec une MAE très basse, et semble également confirmer l'interprétation graphique, avec $b_{1} \gg b_{2}$, et permet d'évaluer les hyperparamètres optimaux permettant de maximiser l'indice de Youden pondéré ($minNode = `r BI_modelquad_Rborist_top["minNode"]`$ et $predFixed = `r BI_modelquad_Rborist_top["predFixed"]`$) afin de lancer la prédiction sur un modèle optimisé. 
\paragraph*{}
...
\end{array}
\right. \\
\quad MAE = `r signif(BI_MAE_ranger,4)`
\end{array}$$
\end{minipage}
\begin{minipage}{0.4 \textwidth}
$$\left \{
...
\paragraph*{}
La modélisation quadratique correspond bien aux données expérimentales, avec une MAE extrêmement basse, et confirme par ailleurs les conclusions de l'interprétation graphique, avec $b_{1} \gg b_{2}$. Une optimisation des hyperparamètres grâce à la modélisation quadratique  ($min.node.size = `r BI_modelquad_ranger_top["min.node.size"]`$, $mtry = `r BI_modelquad_ranger_top["mtry"]`$ et $splitrule = `r BI_modelquad_ranger_top[1,"splitrule"]`$) a donné d'excellents résultats\ :
$$\left \{
\begin{array}{l}
