...
Ce modèle, bien que relativement simple, donne encore des résultats très honorables, avec un kappa ($\kappa_{max} =$ `r round(max(MULESP_fit_rpart_cp_resultats["Kappa"]),3)` et une précision $R_{max} =$ `r round(max(MULESP_fit_rpart_cp_resultats["Accuracy"]),3)`).
