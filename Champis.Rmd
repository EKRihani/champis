...
```{r, lda_pda, echo = FALSE, fig.height = 3, fig.cap = "Performances des modèles lda2 (à gauche) et pda (à droite)"}
plot(ggarrange(
   ncol = 2,
...
```
\paragraph*{}
Comme l'illustre la figure \ref{fig:lda_pda}, les performances des modèles PDA et LDA sont très proches, et relativement constantes sur la totalité de l'espace expérimental de leurs hyperparamètres.
\paragraph*{}
Le paramètre *dimen* du modèle lda2 ne semble en effet pas avoir d'effet significatif sur ses performances, avec un index J de Youden pondéré relativement constant ($\bar{J_{w}} = `r round(mean(BI_fit_lda2_dim_resultats[,"Jw"]), 3)`$). 
\paragraph*{}
...
\paragraph*{}
L'hyperparamètre *degree* du modèle gamLoess a un impact mineur sur ses performances, avec un écart entre indices de Jouden maximal et minimal de $\Delta J_{w} = `r round(max(BI_fit_gamLoess_degree_resultats[,"Jw"]) - min(BI_fit_gamLoess_degree_resultats[,"Jw"]), 3)`$.
\paragraph*{}
L'hyerparamètre *span* affecte marginalement la sensibilité de gamLoess, avec une valeur optimale de $span = `r BI_fit_gamLoess_span_resultats[which.max(BI_fit_gamLoess_span_resultats[,"Jw"]), "span"]`$, aboutissant à un indice de Youden maximal de $J_{w_{max}} = `r round(max(BI_fit_gamLoess_span_resultats["Jw"]), 3)`$.
\paragraph*{}
Les performances de ce modèle n'atteingnent pas le critère posé pour notre étude. Le modèle additif généralisé s'est avéré inférieur aux modèles d'analyse discriminante linéaire, aussi bien en en sensibilité (`r round(max(BI_fit_gamLoess_span_resultats["Sens"]), 3)` vs `r round(mean(c(BI_fit_lda2_dim_resultats$Sens, BI_fit_pda_lambda_resultats$Sens)), 3)`) qu'en spécificité (`r round(max(BI_fit_gamLoess_span_resultats["Spec"]), 3)` vs `r round(mean(c(BI_fit_lda2_dim_resultats$Spec, BI_fit_pda_lambda_resultats$Spec)), 3)`).
\paragraph*{}
...
```{r, echo = FALSE}
kable(BI_fit_rpart_cp_resultats[,c("cp","Jw", "Sens", "Spec", "ROC")], digits = 5, caption = "Performance du modèle CART (rpart)")
```

\paragraph*{}
Le modèle CART le plus simple n'atteint jamais les performances requises sensibilité requise $Jw \geq `r Jw_min`$. Toutefois, ce modèle s'en approche, et donne de très bons résultats globaux, avec :
$$\left \{
\begin{array}{l}
J_{w_{max}} = `r round(max(BI_fit_rpart_cp_resultats["Jw"]),3)` \\
Spe_{Jw_{max}} = `r round(BI_fit_rpart_cp_resultats[which.max(BI_fit_rpart_cp_resultats[,"Jw"]),"Spec"],3)` \\
Sen_{Jw_{max}} = `r round(BI_fit_rpart_cp_resultats[which.max(BI_fit_rpart_cp_resultats[,"Jw"]),"Sens"],3)` \\
\end{array}
\right.$$
...
```{r, echo = FALSE}
kable(BI_fit_c50tree_resultats[,c("Jw", "Sens", "Spec", "ROC")], digits = 5, caption = "Performance du modèle C5.0 tree")
```

\paragraph*{}
De manière assez surprenante, bien que ne disposant d'aucun hyperparamètre, ce modèle a donné d'excellents résultats sans optimisation nécessaire, avec de très hautes sensibilité et spécificité. Toutefois, le modèle C5.0 tree n'a pas rempli l'objectif posé par le critère $J_w \geq `r Jw_min`$.
...
$$\left \{
\begin{array}{l}
J_{w_{max}} = `r round(BI_fit_ranger_best_results["Jw"],5)` \\
Spe_{Jw_{max}} = `r round(BI_fit_ranger_best_results["Spec"],5)` \\
Sen_{Jw_{max}} = `r round(BI_fit_ranger_best_results["Sens"],5)` \\
\end{array}
\right.$$
