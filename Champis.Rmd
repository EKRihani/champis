...
\FloatBarrier
Un autre caractère assez évident est la forme du chapeau. La figure \ref{fig:AnatChapeaux} présente les formes de chapeau principales pour les sporophores de champignons de morphologie agaricoïde,\footnote{La morphologie agaricoïde correspond à la "représentation habituelle" du champignon et se caractérise par la présence d'un chapeau, d'un stipe, et d'un hyménophore à lames.} mais bien d'autres géométries existent : consoles, croûtes...
\FloatBarrier
\begin{figure}
   \centering
...
La première étape est de définir l'indice de Youden pondéré, et les pondérations respectives de la sensibilité et de la spécificité.\footnote{cf. section \ref{chapitre:perf}, page \pageref{chapitre:perf}} Cette définition n'est pas nécessaire pour les classifieurs multiclasse, le kappa ($\kappa$) et l'indice de Rand ($R$) étant des métriques évaluées nativement par la librairie *caret*.
\small
```{r, eval = FALSE}
BI_w <- 10
...
\paragraph*{}
L'insertion de tableaux sous Rmarkdown s'effectue grâce à la fonction \verb|kable|, et les paramètres de style peuvent être configurés plus finement à l'aide de la fonction \verb|kable_styling| apportée par la librairie *kableExtra*. Nous avons notamment ajouté des tons bicolores pour le fond des lignes du tableau (\verb|striped|), et bloqué la position des tableaux vis-à-vis du texte (\verb|hold_position|).
\paragraph*{}
