...
- Optimisation de la "résolution" (maximiser $D^{2}/S$)
...
```{r, dev = "png", dev.args=list(bg="transparent"), fig.height = 5.8,  fig.cap = "Performances de rpart, en fonction du paramètre de complexité"}
...
- Performances médiocres ($\kappa \ll 0.80$), limites atteintes

## Forêts aléatoires

```{r, dev = "png", dev.args=list(bg="transparent"), fig.height = 5.8, fig.cap = "Performances des modèles Ranger (à g.) et Rborist (à d.)"}
ggarrange(
   ncol = 2,
...
```{r, fig.cap = "Matrice de confusion erreurs de Ranger (pred. à g., ref. en h.)"}
kable(MULESP_CMerreurs_ranger, align = "c") %>%
